...
7. `site-index` — **the correct-by-construction model document AND the triangulation base case.** The index exists as two *authored, synchronized* source representations plus generated outputs:
   ...
   **Served filename: decided by experiment (2026-08-03), refuting the documentation-derived assumption.** An artifact containing *only* `index.xhtml` was actually deployed (run 30800978555): the site root returned **HTTP 200 serving that page** — GitHub Pages **does** resolve `index.xhtml` as a directory index — with **`Content-Type: application/xhtml+xml`** on both the root and the direct path. Decision: published pages are `.xhtml`, served as `application/xhtml+xml`; browsers therefore parse them with the strict, fail-fast XML parser — the XML-serialization discipline holds end-to-end, from build gates through the reader's browser. (xslTNG's `$html-extension` parameter set to `.xhtml` at implementation; the polyglot `index.html` fallback is unnecessary and dropped.)
...
**Title: never generated — authored only, via two channels.** A heading is not a title; deriving a title from any non-title syntax element is undecidable in general (proven for this corpus by a counterexample pair: two h3-first documents with identical structural signatures where the first heading is the title in one and a body heading in the other). No heading-derivation, no filename-derivation, no doctoring scripts — the machine proposes nothing. Authored titles arrive only through:
1. **DocBook `info/title`** — native, for authored-DocBook sources (the model index; reconstructed documents).
2. **YAML front matter `title:`** — the Jekyll/GitHub-Pages-standard structured-metadata convention for Markdown sources, parsed by commonmark-java's official `yaml-front-matter` extension and copied verbatim into `info/title`. Authoring, not generation: one reviewed line per document, making legacy publishability an incremental curation act rather than requiring full reconstruction. (Jekyll itself is *not* used — artifact deployment bypasses it; only the convention is adopted. Front matter `date:` is the designated future channel for correcting the import-date defect, census check 3 — not scoped in this increment.)

Bare CommonMark has no title syntax, and the Google Docs origin *had* a Title style the authoring process never used — so today title absence is a **defect of the source documents**, universal (123/123, zero front matter anywhere — verified). The pipeline emits DocBook's grammar-minimum `<title/>` (empty — grammar-valid, verified empirically) and the **non-empty-title policy Schematron rule (citing W3C ACT 2779a5) fails every legacy document, loudly**, until a title is authored. (Verified: the pinned XHTML5 RELAX NG grammar does *not* constrain title emptiness — `title.inner = (text)` — vnu's equivalent check is Java outside the grammar; hence the project Schematron rule is the enforcement point.)
...
1. **Title class** (survey): DocBook-native title / front-matter title / H1-first / H2-first / H3-first / headingless — classes only, mechanically decidable; **no severity ranking** (a judgment, not a decidable property). Known: 0 / 0 / 98 / 3 / 21 / 1; only the first two classes are publishable-eligible.
...
| `org.commonmark:commonmark-ext-yaml-front-matter` | same version as core | official extension; authored-title channel; determinism verified at plan time like all extensions |
...
- Document atomization (XInclude shell/fragment).
- Front-matter fields beyond `title:` (the `date:` authorship-correction channel is designated but deferred; no broader front-matter schema).
- Performing the title curation itself (adding front matter to the 123 documents) — that is triage-driven content work the census exists to prioritize, not pipeline work.
...
- Title generation (first-heading and filename variants) — struck: heading ≠ title; undecidable; absence is a source defect to surface, not patch. Replaced by the authored-only channels: DocBook `info/title` and Jekyll-convention YAML front matter (the platform's own standard solution to Markdown's missing title syntax).
