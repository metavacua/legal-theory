...
### Task 7: deleted, fully superseded 2026-07-26

This task proposed native DocBook bibliography vocabulary (`<bibliography>`/`<biblioentry>`/
`<bibliomixed>`) and CiTO `xlink:arcrole` citation-typing, but only for the one generated,
consolidated `docs/bibliography/references.xml` this plan's Task 6 (predecessor code,
`build_bibliography.py`) produces -- not for the 119 corpus documents' own source citations, which
it explicitly left on the informal `works-cited`/`<listitem>` convention. That is the "adapt around
the non-standard part" pattern this whole standardization effort exists to correct.

The corrected direction -- converting every citation to a real, `xml:id`-tagged `<biblioentry>` at
its own source document, not a separately-generated side document -- is
`docs/superpowers/specs/2026-07-26-citation-standardization-design.md` and its implementation plan,
`docs/superpowers/plans/2026-07-26-citation-standardization-phase1.md` (Phase 1 complete; bulk
corpus-wide conversion is Phase 2, see `.superpowers/sdd/progress.md`).

This task is deleted, not merely superseded-in-place: none of its steps should run. Task 9 and
Task 11 below were originally written assuming Task 7 would produce `docs/scripts/corpus_ast.py`
(a shared `lxml`-based parse/serialize/root-construction module) and migrate
`build_bibliography.py`'s XML emission onto `lxml.etree` with a `BibliographyEntry.bib_key` field
and `emit_docbook_tree()`. None of that was built. Task 9 and Task 11 need a re-scoping pass before
execution to account for Task 7 no longer existing -- tracked as pending, not resolved by this
edit.

Separately, and independently of the citation-standardization correction above: this task's own
Step 1 targeted `docs/schema/docbook-corpus.rnc`, which no longer exists -- Phase 1 of the DocBook-
native corpus standardization project retired that custom schema entirely in favor of the real,
fetched OASIS DocBook 5.2 RELAX NG grammar (`.cache/docbook-5.2/docbookxi.rnc`, validated via
`jing`). Any re-scoped successor to this task must validate against that real grammar, not a
project-owned schema file.
