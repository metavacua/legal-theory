...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/census">
    <!-- Carrier wrapper, no namespace: this element is never DocBook, only a vehicle for
         inject-census.xsl to select the two DocBook-namespaced children out of. A DOM
         Document permits exactly one document element, so census-to-docbook.xsl's output
         (para + informaltable) must have a single root here, not two top-level siblings. -->
    <census-fragment xmlns="">
      <para xmlns="http://docbook.org/ns/docbook">Generated <xsl:value-of select="@generated"/>. Documents:
